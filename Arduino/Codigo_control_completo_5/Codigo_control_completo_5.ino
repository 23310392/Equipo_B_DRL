...
const int PWM_HOVER = 1450; // Aprox 45% de potencia para sostener 900g
const int VELOCIDAD_RAMPA = 2; // Incremento de microsegundos por ciclo de 10ms
...
// Constantes PID ajustadas para Frame 450 (900g, brazos 21cm)
...
void setup() {
  ...
  // 2. CONFIGURAR Y ARMAR LOS ESCs (Secuencia de inicio)
  
  // Asignar los 4 timers por hardware del ESP32 (Obligatorio para 4 motores)
  ESP32PWM::allocateTimer(0);
  ESP32PWM::allocateTimer(1);
  ESP32PWM::allocateTimer(2);
  ESP32PWM::allocateTimer(3);

  // Fijar la frecuencia EXACTA a 50Hz que requieren los ESC de 30A
  ...
  // Damos 4 segundos al inicio para que los ESCs reciban el pulso y terminen de pitar
  delay(4000); 
  ...
  // Damos 4 segundos al inicio para que los ESCs reciban el pulso mínimo y se calibren/armen
  delay(4000); 
  Serial.println("ESCs Armados y Listos.");
  ...
  if (Wire.endTransmission() != 0) {
    ...
    Wire.beginTransmission(MPU_ADDR);
    Wire.write(0x1A); Wire.write(0x03); // Filtro paso bajo a 42Hz
    Wire.endTransmission(true);
...
void loop() {
  ...
  if (dt >= 0.01) { // 100Hz
    ...
    if (estadoDron != SUELO) {
      ...
      // Control de altura manual (permite variar +-200us sobre la base)
